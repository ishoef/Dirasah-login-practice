const MMTAMIL = {
    ...
    this.uidRegex = new RegExp(`${MMTAMIL.PREFIX}[\\d]{${MMTAMIL.LENGTH}}`, 'gi')
    
    }
    
    createUidByNumber(number) {
    
    return MMTAMIL.PREFIX + (10 ** MMTAMIL.LENGTH + number).toString().slice(-MMTAMIL.LENGTH)
    ...
    if (!results) throw Error(`No MMTAMIL found in the current confirmation message with regex ${this.uidRegex}.`)
    ...
    const nextUidNumber = Number(currentUid.replace(MMTAMIL.PREFIX, "")) + 1
    ...
    let uidHeaderIndex = headers.indexOf(MMTAMIL.HEADER)
    
    if (uidHeaderIndex === -1) {
    
    uidHeaderIndex = headers.length
    
    this.sheet.getRange(1, uidHeaderIndex + 1).setValue(MMTAMIL.HEADER)
